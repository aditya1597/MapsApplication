...
public class request_help extends AppCompatActivity implements OnMapReadyCallback {
    GoogleMap mMap;
    @Override
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        final BitmapDescriptor icon= BitmapDescriptorFactory.fromResource(R.drawable.volunteer);
        db.collection("volunteer_location_data").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if(task.isSuccessful())
                {
                    for (DocumentSnapshot document : task.getResult()) {
                        ...
                        mopt.icon(icon);
                        mopt.title("Volunteer Contact Number:"+document.getId().toString());
                        mMap.addMarker(mopt);
                    }
                }
                else {
